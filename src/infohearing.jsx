...
【金融用語の正しい読み方（音声出力時の発音）】
- iDeCo は「イデコ」と発音してください（「アイ・ディー・イー・シー・オー」や「アイデコ」ではない）
- NISA は「ニーサ」と発音してください
- REIT は「リート」と発音してください
- つみたてNISA は「ツミタテニーサ」
- 新NISA は「シンニーサ」
- 英文字の金融用語は全てカタカナの日本式読みで発音してください
...
function filterJapaneseOnly(text) {
  ...
  // 括弧内に英字や引用符("項目名")が含まれるもの (Name), （男性）, ("男性"), (「氏名」) などを除去
  filtered = filtered.replace(/\s*[\(（][^\)）]*[a-zA-Z][^\)）]*[\)）]/g, "");
  filtered = filtered.replace(/\s*[\(（][^\)）]*["'"「]+[^\)）]*[\)）]/g, "");

  // 英語の短縮形や孤立断片（I'm, I's, It's, don't, we'll, you're 等）を除去
  filtered = filtered.replace(/\b[A-Za-z]+['’'][A-Za-z]+\b/g, "");
  filtered = filtered.replace(/\b[A-Za-z]['’']\s?/g, "");
  ...
  // 孤立した1文字の英字（I, a等）と残骸を除去
  filtered = filtered.replace(/(^|\s)[a-zA-Z](?=\s|[,.、。！？]|$)/g, "$1");
  // ダッシュ・繰り返し記号（–, —, .., ,,）で孤立しているものを除去
  filtered = filtered.replace(/(^|\s)[–—]+(\s|$)/g, " ");
  filtered = filtered.replace(/\s*\.{2,}\s*/g, "");
  filtered = filtered.replace(/\s*,{2,}\s*/g, "");

  // 先頭に日本語以外のノイズがある場合、最初の日本語文字まで削除
  const firstJp = filtered.search(/[぀-ゟ゠-ヿ一-鿿]/);
  if (firstJp > 0) {
    const leading = filtered.substring(0, firstJp);
    // 先頭部に日本語文字がなく、句読点・空白・記号・英字のみの場合は削除
    if (!/[぀-ゟ゠-ヿ一-鿿]/.test(leading)) {
      filtered = filtered.substring(firstJp);
    }
  }
    ...
    if (data.serverContent) {
      ...
      if (sc.modelTurn?.parts) {
        ...
        for (const part of sc.modelTurn.parts) {
          ...
          // NOTE: part.text はAIの思考トレースであり、実際に音声化される内容ではない。
          // 表示用テキストは outputTranscription のみを利用する（音声の文字起こし）。
        }
      }
